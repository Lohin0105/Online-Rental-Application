...
import { PriceCalculatorComponent } from '../../../shared/components/price-calculator/price-calculator.component';

@Component({
    standalone: true,
  selector: 'app-property-form',
    imports: [FormsModule, RouterLink, PriceCalculatorComponent],
    template: `
            ...
            <section class="form-section">
              ...
              <div class="form-group">
                <label class="form-label">
                  Description
                  <button type="button" class="ai-enhance-btn" (click)="enhanceDescription()" [disabled]="!formData.description || enhancing()" title="Enhance with AI">
                    <span class="material-icons-outlined">{{ enhancing() ? 'hourglass_empty' : 'auto_awesome' }}</span>
                ...
                <textarea 
                  class="form-input"
                  rows="6"
                  placeholder="Describe your property... (Write a basic description, then click 'Enhance with AI')"
...
export class PropertyFormComponent implements OnInit {
  ...
  async enhanceDescription() {
    if (!this.formData.description || this.formData.description.trim().length < 10) {
      this.notification.error('Please write a basic description first (at least 10 characters)');
      return;
    }

    this.enhancing.set(true);
    try {
      const enhanced = await this.aiService.enhancePropertyDescription(this.formData.description);
      this.formData.description = enhanced;
      this.notification.success('Description enhanced with AI!');
    } catch (error) {
      this.notification.error('Failed to enhance description. Please try again.');
    } finally {
